...
class Pipeline:
    ...
    def dictionary_and_embedding(self, input_file, size):
        ...
        ### ... codigo anterior
        # Esta es la línea que arreglaste antes:
        train_ids = pd.concat([pairs['id1'], pairs['id2']]).unique()

        # --- AÑADE ESTO (BLOQUE DE SEGURIDAD) ---
        # Filtramos para quedarnos SOLO con los IDs que existen realmente en nuestros datos parseados
        valid_ids = set(self.sources['id'])
        train_ids = [tid for tid in train_ids if tid in valid_ids]
        # ----------------------------------------
    ...
    def merge(self, data_path, part):
        ...
        # --- CORRECCIÓN DE RUTA Y CREACIÓN DE CARPETA ---
        # 1. Construimos la ruta bien (añadiendo la barra que faltaba entre root y language)
        # Esto convertirá "datajava" en "data/java"
        save_dir = self.root + '/' + self.language + '/' + part

        # 2. Creamos la carpeta si no existe (¡IMPORTANTE!)
        import os
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)

        # 3. Guardamos el archivo dentro de esa carpeta
        print(f"Guardando en: {save_dir}/blocks.pkl")
        df.to_pickle(save_dir + '/blocks.pkl')
        # -----------------------------------------------
